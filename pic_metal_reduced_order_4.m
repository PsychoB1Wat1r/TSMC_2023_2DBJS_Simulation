...
%% (异步) H2 异步双模滤波器参数
% 异步滤波器模态1
At1 = [ 0.2349   -0.1058    0.0309   -0.2918;
        0.0004   -0.0005    0.0080    0.0004;
        0.2177   -0.1143   -0.4947   -0.3034;
        0.0281    0.0097    0.3749    0.0134];
Bt1 = [-0.4298;
       -1.4163;
       -1.1794;
       -0.8999];
Jt1 = [0.0346   -1.0098   -0.1483   -0.9809];
Lt1 = 1.2357;
% 异步滤波器模态2
At2 = [ 0.2645   -0.1071    0.4749   -0.2886;
        0.0120   -0.0039    0.1508    0.0068;
        0.2111   -0.1060   -0.6307   -0.3196;
        0.0072    0.0158    0.1074    0.0019];
Bt2 = [ 0.2062;
       -1.2057;
       -1.3901;
       -1.2933];
Jt2 = [0.0354   -1.0099   -0.1483   -0.9806];
Lt2 = 1.2358;
%% 系统伯努利过程 alpha 系统切换
...
xf1 = zeros(Nt, Nx);  xf2 = zeros(Nt, Nx);  xf3 = zeros(Nt, Nx);  xf4 = zeros(Nt, Nx); % 滤波输出序列存储数组(滤波后)
...
% x3(1:Nt, 1) = -1.5;  x4(1:Nt, 1) = -2.0;  x5(1:Nt, 1) = -2.1;  xf3(1:Nt, 1) = 1.8;  xf4(1:Nt, 1) = 1.7; % 垂直方向
%% 系统 滤波器 模态初值 边界条件
...
for i = 1:Nt
    for j = 1:Nx
        ...
        xf1(i+1,j) = At(1,:)*[xf1(i,j); xf2(i,j); xf3(i,j); xf4(i,j)] + Bt(1,1)*yf(i,j);
        xf2(i+1,j) = At(2,:)*[xf1(i,j); xf2(i,j); xf3(i,j); xf4(i,j)] + Bt(2,1)*yf(i,j);
        xf3(i,j+1) = At(3,:)*[xf1(i,j); xf2(i,j); xf3(i,j); xf4(i,j)] + Bt(3,1)*yf(i,j);
        xf4(i,j+1) = At(4,:)*[xf1(i,j); xf2(i,j); xf3(i,j); xf4(i,j)] + Bt(4,1)*yf(i,j); % 全阶滤波器信号
        zsim(i,j) = Jt*[xf1(i,j); xf2(i,j); xf3(i,j); xf4(i,j)] + Lt*yf(i,j); % 估计信号
        zerror(i,j) = z(i,j) - zsim(i,j); % 滤波误差
        zsum(i,j) = sqrt(norm(zerror(1:i, 1:j), 2)); % 误差信号的 2 范数
    end
end
%% 滤波误差
figure('name', '滤波误差信号');
...
axis([0 T 0 Lx -0.5 1]);  grid on;  grid minor;  box on;
%% 性能比较
figure('name', '性能比较');
...
axis([0 T 0 Lx 0.2 0.9]);  grid on;  grid minor;  box on;
%% H2 范数求解
disp('****************************');
disp('****水平/垂直 爱趣 2 范数****');
disp('****************************');
