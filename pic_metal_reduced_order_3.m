...
%% (异步) H2 异步双模滤波器参数
% 异步滤波器模态1
At1 = [ 0.2136   -0.1609    0.7897;
       -0.0099    0.0106    0.2008;
        0.1781   -0.0945   -0.1953];
Bt1 = [-0.5063;
       -1.4562;
       -1.4938];
Jt1 = [0.1838   -1.0432   -0.7324];
Lt1 = 0.9665;
% 异步滤波器模态2
At2 = [ 0.2065   -0.2059    0.9624;
       -0.0035   -0.0119    0.2441;
        0.1715   -0.0481   -0.4817];
Bt2 = [-0.0206;
       -1.2279;
       -2.0398];
Jt2 = [0.1865   -1.0444   -0.7194];
Lt2 = 0.9826;
%% 系统伯努利过程 alpha 系统切换
...
xf1 = zeros(Nt, Nx);  xf2 = zeros(Nt, Nx);  xf3 = zeros(Nt, Nx); % 滤波输出序列存储数组(滤波后)
...
% x3(1:Nt, 1) = -1.5;  x4(1:Nt, 1) = -2.0;  x5(1:Nt, 1) = -2.1;  xf3(1:Nt, 1) = 1.8; % 垂直方向
%% 系统 滤波器 模态初值 边界条件
...
for i = 1:Nt
    for j = 1:Nx
        ...
        xf1(i+1,j) = At(1,:)*[xf1(i,j); xf2(i,j); xf3(i,j)] + Bt(1,1)*yf(i,j);
        xf2(i+1,j) = At(2,:)*[xf1(i,j); xf2(i,j); xf3(i,j)] + Bt(2,1)*yf(i,j);
        xf3(i,j+1) = At(3,:)*[xf1(i,j); xf2(i,j); xf3(i,j)] + Bt(3,1)*yf(i,j); % 降阶滤波器信号
        zsim(i,j) = Jt*[xf1(i,j); xf2(i,j); xf3(i,j)] + Lt*yf(i,j); % 估计信号
        zerror(i,j) = z(i,j) - zsim(i,j); % 滤波误差
        zsum(i,j) = sqrt(norm(zerror(1:i, 1:j), 2)); % 误差信号的2范数
    end
end
%% 滤波误差
figure('name', '3 阶 滤波误差信号');
...
disp('****水平/垂直 爱趣 2 范数****');
disp('****************************');
Norm_order_3 = sqrt(norm(zerror(1:i,1:j), 2))
